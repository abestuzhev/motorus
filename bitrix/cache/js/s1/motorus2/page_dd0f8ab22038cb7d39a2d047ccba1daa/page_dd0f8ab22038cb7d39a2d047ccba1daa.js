...
/* End */
;
; /* Start:/bitrix/components/bitrix/search.tags.input/templates/.default/script.js*/
var Errors = {
...
window.LHEPostForm.prototype = {
	initFiles : function(formID, params)
	{
		...
		this.controllerAdjustWithInlineTags = BX.delegate(function(obj)
		{
			...
			if (needToReparse && needToReparse.length > 0 && this.oEditor && this.parser[parser])
			{
				...
				var content = this.oEditor.GetContent(), reg;
				if (parser == 'disk_file' && obj.dataFit)
				{
					for (var ii in obj.dataFit)
					{
						reg = new RegExp('\\&\\#91\\;DOCUMENT ID=(' + ii + ')([WIDTHHEIGHT=0-9 ]*)\\&\\#93\\;','gim');
						if (obj.dataFit.hasOwnProperty(ii))
						{
							content = content.replace(reg, '[' + this.parser[parser]["tag"] + '=' + obj.dataFit[ii] + '$3]');
						}
					}
				}
	...
	OnWDSelectFileDialogLoaded : function(obj)
	{
		...
		{
			id = parseInt(obj.values[ii].getAttribute("id").replace(c["prefixNode"], ""));
			xmlID = parseInt(obj.values[ii].getAttribute("bx-attach-xml-id"));
			if (arID['id' + id])
				continue;
			arID['id' + id] = "Y";
			if (id > 0)
	...
	bindToFile : function(id)
	{
		var
			f = this.checkFile(id),
			intId = parseInt(id.replace(/\D+/gi, '')),
			img_wrap,
			c = this.controllers[f.cid],
			node = BX.findChild((c ? c.node : window), {attr : { id : c["prefixNode"] + intId }}, true);
		if (f && node)
		{
	...
	checkFile : function(id, cid, result, isNew)
	{
		...
		if (typeof result == "object" && result !== null)
		{
			...
			if (result.isImage && result.storage == 'webdav' && this.arSize && result.element_thumbnail)
			{
	...
	{
		var
			lheName = this.params['LHEJsObjName'];
	...
	InitCustomEditorEventHandlers: function()
	{
		...
		if (!editor.mainPostFormCustomized)
		{
			...
			BX.addCustomEvent(
				editor,
				'OnIframeKeydown',
				function(e/*, keyCode, command, selectedNode*/)
				{
					if (window["bxPFParser"])
					{
						window["bxPFParser"](e, editor, formID);
					}
				}
			);
		}
...
window.LHEPostForm.reinitData = function(editorID, text, data, controller)
{
	...
	var ii, id, dialogName, storage, ccid,
		node,
		res,
		tmp,
		node1,
	...
	if (!controller)
	{
		...
		if (data)
		{
			...
			for (ii in data)
			{
				...
				else if (data[ii]["USER_TYPE_ID"] == "webdav_element" || data[ii]["USER_TYPE_ID"] == "disk_file")
				{
	...
	var cObjS1 = [], dataC, dataD, dataE, dataF, dataG, defFun = [], ready = cObjS.length;
	...
	for (ii = 0; ii < cObjS.length; ii++)
	{
		...
		else if (cObj["userTypeId"] == "webdav_element")
		{
			if (cObj["arguments"].length > 0)
			{
				...
				defFun.push(function() { cObjS[ii]["obj"].WDFD_SelectFile({}, {}, dataC) });
			}
		}
		else if (cObj["userTypeId"] == "disk_file")
		{
			if (cObj["arguments"].length > 0)
			{
				...
				defFun.push(function() { cObjS[ii]["obj"].selectFile({}, {}, dataD) });
			}
		}
		else if (cObj["arguments"] && cObj["arguments"] !== null && typeof cObj["arguments"] == "object")
		{
			...
			if (cObj["cid"])
			 ...
			 defFun.push(function() {
				for (var ij = 0; ij < dataF.length; ij++)
				{
					cObjS[ii]["obj"].agent.values.push(dataF[ij]);
					cObjS[ii]["obj"].agent.ShowAttachedFiles();
				}
			 });
		}
...
/*
							if (handler.controllers[i].parser == 'disk_file' && obj.dataFit)
							{
								for (var ii in obj.dataFit)
								{
									reg = new RegExp('\\&\\#91\\;DOCUMENT ID=(' + ii + ')([WIDTHHEIGHT=0-9 ]*)\\&\\#93\\;','gim');
...
// remove block
	...
	window.bxPFParser = function(e, editor, formID)
	{
		...
		if(MPFMention["listen"] === true)
		{
			if(keyCode == 8) // backspace
			{
				setTimeout(function()
				{
					...
					if(txt === undefined || txt === null || txt.length === 0 || (txt.lastIndexOf("+", range.endOffset) == -1 && txt.lastIndexOf("@", range.endOffset) == -1))
					{
		...
		if(MPFMention["listen"] === true)
		{
			...
			else
			{
				setTimeout(function()
				{
					...
					{
						var
							txtPos = txt.lastIndexOf((MPFMention["plus"] ? "+" : "@"), range.endOffset) + 1,
							txt2 = txt.substr(txtPos, (range.endOffset - txtPos));
	...
	window.BXfpdSelectCallbackMent = function (item, type, search, formID, editorId, bNeedComa)
	{
		var editor = window.BXHtmlEditor.Get(editorId);
		...
		if(type == 'users' && item && item.entityId > 0 && editor)
		{
			if(editor.GetViewMode() == 'wysiwyg') // WYSIWYG
			{
				...
				var
					txt = (range ? range.endContainer.textContent : ''),
					txtPos = txt.lastIndexOf((MPFMention["plus"] ? "+" : "@"), range.endOffset);

				txtPos = (txtPos >= 0 ? txtPos : range.endContainer.length);
				txtPos = (txtPos >= 0 ? txtPos : 0);

				var
					node = doc.getElementById('bx-mention-symbol'),
				...
				if (!node)
				{
					...
					else if(txtPosEnd > 0 && !range.endContainer.length)
						txtPosEnd = 0;
					else if(txtPosEnd > range.endContainer.length)
						txtPosEnd = range.endContainer.length;

					rng.setStart(range.endContainer, txtPos);
					rng.setEnd(range.endContainer, txtPosEnd);

					node = BX.create('SPAN', {props: {className: 'bxhtmled-metion'}}, doc);
					editor.selection.SetSelection(rng);
					editor.selection.Surround(node, rng);
				}

				editor.util.ReplaceNode(node, mention);
	...
	window.MPFMentionInit = function(formId, params)
	{
		...
		{
			window.BXfpdSelectCallbackMent(item, type, search, formId, params["editorId"]);
		}
				...
				'closeDialog' : function()
				{
					...
					if(editor)
					{
						...
						if (node)
						{
							...
							{
								node.innerHTML = node.innerHTML.substr(1);
								editor.util.ReplaceWithOwnChildren(node);
							}
						}
						...
						function(e)
						{
							if(MPFMention["listen"] !== true)
							{
								...
								if(editor.GetViewMode() == 'wysiwyg' && doc)
								{
									...
									var node = doc.getElementById('bx-mention-symbol');
									if (node)
									{
										editor.selection.SetAfter(node);
										if (node.innerHTML == '@')
									...
									editor.InsertHtml('<span id="bx-mention-symbol">@</span>');
								}
...
; /* /bitrix/components/bitrix/search.tags.input/templates/.default/script.js*/
