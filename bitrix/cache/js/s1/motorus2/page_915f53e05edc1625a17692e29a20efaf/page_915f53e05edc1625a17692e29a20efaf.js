...
; /* Start:/bitrix/templates/.default/components/bitrix/catalog.smart.filter/visual_vertical/script.js*/
...
{
	if (BX.hasClass(input, "max-prop") || BX.hasClass(input, "min-prop")){
		var propsContainer = BX.findParent(BX(input.id), {"tag" : "div", "class" : "bx_filter_container"}),
			checkboxes = BX.findChildren(BX(propsContainer.id), {tagName: 'input', className: "check-prop"}, true),
			rangeMin = BX.findChild(BX(propsContainer.id), {tagName: 'input', className: "min-prop"}, true),
			rangeMax = BX.findChild(BX(propsContainer.id), {tagName: 'input', className: "max-prop"}, true);
		
		for (var checkbox in checkboxes)
		{
			var value = checkboxes[checkbox].getAttribute("data-value");
			if (value >= rangeMin.value && value <= rangeMax.value) {
				BX.adjust(checkboxes[checkbox], { props: { checked: 'checked' } });
			} else {
				BX.adjust(checkboxes[checkbox], { props: { checked: '' } });
			}
		}	
		if(this.timer)
...
JCSmartFilter.prototype.controlFilter = function(input)
{
...
JCSmartFilter.prototype.reload = function(input)
{
	...
	if(this.form)
	{
		...
		var elements =  BX.findChildren(this.form, {'tag':'input'}, true),
			lastElements =  BX.findChildren(this.form, {className : "last"}, true),
			mergedArray = elements;
		if (lastElements){
			for (var i = 0; i < lastElements.length; i++) {
				mergedArray = mergedArray.concat(lastElements[i]);
			}
			this.gatherInputsValues(values,mergedArray);
		} else {
			this.gatherInputsValues(values,elements);
		}
		
                var url = this.buildUrlParams({ ELEMENT_COUNT: 0, FILTER_URL: this.ajaxURL });
                BX("set_filter").setAttribute("data-url", BX.util.htmlspecialcharsback(url));
...
JCSmartFilter.prototype.buildUrlParams = function(result)
{
	containers = BX.findChildren(BX("bx_filter"), { className : "bx_filter_container"},true);
	urlParams = new Array;
	for (var container in containers){
		rangeMin = BX.findChild(containers[container],{ className : "min-prop" },true)
		rangeMax = BX.findChild(containers[container],{ className : "max-prop" },true)
		if (rangeMin && rangeMax){
			urlParams.push("arrFilter_"+rangeMin.getAttribute("data-id-value")+"_MIN"+"=" + rangeMin.value);
			urlParams.push("arrFilter_"+rangeMax.getAttribute("data-id-value")+"_MAX"+"=" + rangeMax.value);
		}
	}

	count = "arrCount="+result.ELEMENT_COUNT;
	urlParams.push(count);
	return result.FILTER_URL + "&" + urlParams.join("&");
}
	...
	if(!!result && !!result.ITEMS)
	{
		...
		if(modef && modef_num)
		{
			...
			if (result.ELEMENT_COUNT>0){
				filterUrl = this.buildUrlParams(result);
				modef.innerHTML = "Выбрано:<span id='modef_num'>"+result.ELEMENT_COUNT+' '+"</span>\
				<span class='arrow'></span>\
				<a href='"+ BX.util.htmlspecialcharsback(filterUrl)+"'>Показать</a>"
				BX("set_filter").setAttribute("data-url",BX.util.htmlspecialcharsback(filterUrl));
			} else {
				modef.innerHTML = "Товаров не найдено<span id='modef_num'></span><span class='arrow'></span><a style='display: none' href=''></a>"
			}
			...
			else
			{
				...
				modef.style.top = this.position.top -3 + 'px';
			}
		}
	}
}
...
/* End */
;; /* /bitrix/templates/.default/components/bitrix/catalog.smart.filter/visual_vertical/script.js*/
